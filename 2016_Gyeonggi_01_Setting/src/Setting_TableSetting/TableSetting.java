...
public class TableSetting {
	...
	public static void CreateTables() throws Exception {
		...
		//ledger
		DBInterface.Stmt.execute("CREATE TABLE `project000`.`ledger` "
	...
	public void CreateData() throws Exception {
		DBInterface.Stmt.execute("ALTER TABLE `project000`.`ledger` "
				+ "ADD INDEX `key1_idx` (`memberid` ASC);");
		DBInterface.Stmt.execute("ALTER TABLE `project000`.`ledger` "
				+ "ADD CONSTRAINT `key1`  "
		...
		InputStream st1 = cl.getResourceAsStream("Data/"+"member.txt");
		InputStream st2 = cl.getResourceAsStream("Data/"+"ledger.txt");
		...
		while((s=in.readLine()) !=null) {
			if(cnt!=1) {
				...
				if(sp.length==7) {
					...
					DBInterface.Stmt.execute("INSERT INTO `project000`.`ledger` (`memberid`, `date`, `division`, `item`, `pay`, `amount`) "
