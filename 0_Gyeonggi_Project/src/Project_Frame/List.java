			...
			public void mouseClicked(MouseEvent e) {
				...
				if(e.getClickCount()==2) {
					if(tab.getSelectedIndex()==0 && model1.getRowCount()!=0) {
						FileSave("수입");
					} else if(tab.getSelectedIndex()==1 && model2.getRowCount()!=0) {
						FileSave("지출");
					}
				}
			}
	...
	public void FileSave(String type) {
		...
		if(res==JOptionPane.YES_NO_OPTION) {
			try {
				...
				while(rs.next()) {
					bw.newLine();
					bw.write(rs.getString(2)+"\t"+rs.getString(3)+"\t"+rs.getString(4)+"\t"+rs.getString(5)+"\t"+rs.getString(6)+"\t"+rs.getString(7)+"\t");
				}
				bw.close();
			} catch (IOException e) {
				// TODO Auto-generated catch block
