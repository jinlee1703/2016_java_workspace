...
public class MemberUpdate extends JFrame {
	...
	public MemberUpdate() {
		...
		JPanel p2 = new JPanel(new GridLayout(5, 1, 5, 5));
		for(int i=0; i<5; i++) {
			t[i] = new JTextField();
			p2.add(t[i]);
		}
		...
		btn1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					DBInterface.Stmt.execute("UPDATE `project000`.`member` SET `pw`='"+t[1].getText()+"', `nname`='"+t[2].getText()+"', `phone_num`='"+t[3].getText()+"', `address`='"+t[4].getText()+"' WHERE `id`='"+t[0].getText()+"';");
					dispose();
